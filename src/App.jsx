...
const C = {
  ...
  light1: "#5a7234", light2:"#6b8641",
  tan1:   "#7a6840", tan2:  "#b89a63",
  accent: "#a6d84a",
  gold:   "#e0b52e",
  alert:  "#e05545",
  muted:  "#97a56f",
...
export default function App() {
  ...
  const BG = "#23262b"; // solid neutral gray for maximum text legibility

  const BOX = (extra={})=>({background:C.sub,border:`1px solid ${C.border}`,borderRadius:8,padding:13,marginBottom:9,...extra});
  const BTN = (bg,clr,extra={})=>({padding:"10px 0",borderRadius:6,background:bg,border:`1px solid ${clr}40`,color:clr,fontSize:11,fontWeight:"bold",cursor:"pointer",letterSpacing:3,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace",...extra});

  return (
    <div style={{minHeight:"100vh",background:BG,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace",color:C.text,display:"flex",flexDirection:"column",maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      {newBadge&&(
        <div style={{position:"fixed",top:14,left:"50%",transform:"translateX(-50%)",zIndex:999,background:`linear-gradient(135deg,${C.gold},#8a6c18)`,borderRadius:8,padding:"11px 18px",display:"flex",alignItems:"center",gap:10,boxShadow:`0 6px 24px rgba(201,162,39,0.45)`,maxWidth:300,width:"90%",border:`1px solid ${C.gold}`}}>
          ...
          <div><div style={{fontWeight:"bold",color:C.dark1,fontSize:11,letterSpacing:2}}>COMMENDATION EARNED</div><div style={{color:C.dark2,fontSize:11,marginTop:2}}>{newBadge.title.toUpperCase()}</div></div>
        </div>
      )}
      ...
      <div style={{position:"relative",zIndex:1,background:C.glass,backdropFilter:"blur(12px)",borderBottom:`1px solid ${C.border}`}}>
        ...
        <div style={{padding:"12px 16px 10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            ...
            <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginTop:2}}>COMMANDER · ENGINEER · FATHER</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:9,letterSpacing:3,color:C.muted}}>DAILY SCORE</div>
            ...
            <div style={{fontSize:9,color:C.muted,letterSpacing:2}}>{doneHabits}/{totalHabits} OBJ</div>
          </div>
        </div>
      </div>
        ...
        {tab==="command"&&(
          <div>
            ...
            <div style={{...BOX({background:"rgba(201,162,39,0.07)",borderColor:"rgba(201,162,39,0.22)",borderLeft:`3px solid ${C.gold}`,marginTop:12})}}>
              <div style={{fontSize:9,letterSpacing:4,fontWeight:700,color:C.gold,marginBottom:5}}>IDENTITY DIRECTIVE</div>
            ...
            <div style={BOX()}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
                <div style={{fontSize:9,letterSpacing:4,fontWeight:700,color:C.muted}}>TODAY'S MISSION</div>
                <div style={{fontSize:10,color:C.accent}}>{doneHabits}/{totalHabits} COMPLETE</div>
              </div>
              ...
              {PILLARS.map(p=>{
                ...
                return(
                  <div key={p.id} onClick={()=>{setActivePillar(p.id);setTab("mission");}}
                    ...
                    <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginBottom:2}}>{p.label}</div>
                    ...
                    <div style={{fontSize:10,color:C.muted,marginTop:4}}>{done}/{p.habits.length} tasks</div>
                  </div>
                );
              })}
            </div>
            ...
            {quitTs?(
              <div onClick={()=>setTab("smoke")} style={{...BOX({background:"rgba(143,196,58,0.06)",borderColor:"rgba(143,196,58,0.18)",cursor:"pointer"})}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontSize:9,letterSpacing:4,fontWeight:700,color:C.muted,marginBottom:4}}>SMOKE-FREE OPERATION</div>
                    <div style={{fontSize:28,fontWeight:900,color:C.accent,lineHeight:1}}>{duration.big}</div>
                    <div style={{fontSize:11,color:C.muted,marginTop:2,letterSpacing:1}}>{duration.unit}</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:9,letterSpacing:3,color:C.muted}}>SAVED</div>
                    <div style={{fontSize:22,fontWeight:900,color:C.accent}}>K{saved.toFixed(0)}</div>
                    <div style={{fontSize:10,color:C.muted,marginTop:2}}>🔥 {streak}d STREAK</div>
                  </div>
                </div>
              </div>
            ):(
              <div onClick={()=>setTab("smoke")} style={{...BOX({background:"rgba(192,57,43,0.07)",borderColor:"rgba(192,57,43,0.2)",cursor:"pointer",textAlign:"center"})}}>
                <div style={{fontSize:11,color:"#e07060",letterSpacing:3}}>🚭 SET QUIT TIME → TAP HERE</div>
              </div>
            )}
            ...
            {journalEntries.length>0&&(
              <div style={{...BOX({background:"rgba(201,162,39,0.04)",borderColor:"rgba(201,162,39,0.12)"})}}>
                <div style={{fontSize:9,letterSpacing:4,fontWeight:700,color:C.muted,marginBottom:5}}>LAST FIELD REPORT</div>
                ...
                <div style={{fontSize:10,color:C.muted,marginTop:5}}>{journalEntries[0].date}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            <div style={{display:"flex",gap:4,margin:"10px 0",overflowX:"auto",paddingBottom:2}}>
              {["timer","health","savings","breathe","badges"].map(st=>(
                <button key={st} onClick={()=>setSmokeTab(st)} style={{padding:"5px 10px",borderRadius:3,background:smokeTab===st?`${C.mid2}90`:"rgba(255,255,255,0.02)",border:smokeTab===st?`1px solid ${C.accent}`:`1px solid ${C.border}`,color:smokeTab===st?C.accent:C.muted,fontSize:10,cursor:"pointer",letterSpacing:2,whiteSpace:"nowrap",fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace"}}>
                  {st.toUpperCase()}
                </button>
              ))}
            </div>
            ...
            {smokeTab==="timer"&&(
              !quitTs?(
                <div style={{...BOX({textAlign:"center",marginTop:8})}}>
                  ...
                  <div style={{fontSize:11,color:C.muted,marginBottom:14,letterSpacing:1}}>Saved permanently on your device</div>
                  <input type="datetime-local" value={setupInput} onChange={e=>setSetupInput(e.target.value)}
                    style={{width:"100%",padding:"9px 10px",borderRadius:5,background:"rgba(255,255,255,0.04)",border:`1px solid ${C.border}`,color:C.text,fontSize:12,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace",boxSizing:"border-box",colorScheme:"dark",marginBottom:10}}/>
                ...
                <div>
                  <div style={{...BOX({background:"rgba(46,61,28,0.7)",borderColor:"rgba(143,196,58,0.25)",textAlign:"center",marginTop:8})}}>
                    <div style={{fontSize:10,letterSpacing:5,fontWeight:700,color:C.muted,marginBottom:7}}>SMOKE-FREE TIME</div>
                    ...
                    {nextMS&&(
                      <div style={{marginTop:12}}>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:C.muted,letterSpacing:1,marginBottom:4}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:9}}>
                    {[
                      ...
                      <div key={s.label} style={{...BOX({padding:"11px 10px",textAlign:"center",marginBottom:0})}}>
                        ...
                        <div style={{fontSize:9,color:C.muted,marginTop:3,letterSpacing:2}}>{s.label}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {tip&&(
                    <div style={{...BOX({background:"rgba(143,196,58,0.06)",borderColor:"rgba(143,196,58,0.18)",fontSize:12,lineHeight:1.8})}}>
                      {tip}
                      {cravings.length>0&&<div style={{marginTop:5,fontSize:10,color:C.muted,letterSpacing:2}}>{cravings.length} CRAVINGS DEFEATED 💪</div>}
                    </div>
                  )}
                  <div style={BOX()}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                      <span style={{fontSize:10,color:C.muted,letterSpacing:2}}>SAVINGS OBJECTIVE</span>
                      <span style={{fontSize:10,fontWeight:"bold",color:C.accent}}>K{saved.toFixed(0)} / K{goalK}</span>
                    </div>
                    <div style={{height:4,background:"rgba(255,255,255,0.05)",borderRadius:2,overflow:"hidden"}}>
                      <div style={{height:"100%",width:`${goalPct}%`,background:`linear-gradient(90deg,${C.mid3},${C.accent})`}}/>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginTop:8}}>
                      <span style={{fontSize:10,color:C.muted,letterSpacing:1}}>GOAL (K):</span>
                      <input type="number" value={goalK} onChange={e=>setGoalK(+e.target.value)}
                        style={{flex:1,padding:"4px 8px",borderRadius:4,background:"rgba(255,255,255,0.04)",border:`1px solid ${C.border}`,color:C.text,fontSize:12,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace"}}/>
                    </div>
                  </div>
                  {!showReset
                    ...
                    :<div style={{...BOX({background:"rgba(192,57,43,0.07)",borderColor:"rgba(192,57,43,0.25)",textAlign:"center"})}}>
                      <p style={{fontSize:11,color:"#e07060",marginBottom:10,letterSpacing:1}}>EVERY ATTEMPT STRENGTHENS YOU. CONFIRM?</p>
            ...
            {smokeTab==="health"&&HEALTH_MILESTONES.map((m,i)=>{
              ...
              return(
                <div key={m.minutes} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${C.border}`,opacity:done||here?1:0.28}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontSize:11,color:done?C.accent:C.muted,letterSpacing:1}}>
                      {m.label.toUpperCase()} {here&&<span style={{color:C.gold,fontSize:9,fontWeight:"bold"}}>← YOU ARE HERE</span>}
                    </div>
            ...
            {smokeTab==="savings"&&(
              <div>
                <div style={{...BOX({background:"rgba(143,196,58,0.07)",borderColor:"rgba(143,196,58,0.2)",textAlign:"center",marginTop:8})}}>
                  <div style={{fontSize:10,letterSpacing:4,fontWeight:700,color:C.accent,marginBottom:5}}>TOTAL SAVED</div>
                  <div style={{fontSize:44,fontWeight:900,color:C.accent}}>K{saved.toFixed(2)}</div>
                  <div style={{fontSize:10,color:C.muted,marginTop:4,letterSpacing:2}}>{cigsAvoided} CIGARETTES NOT BOUGHT</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:9}}>
                  {[
                    ...
                    <div key={s.label} style={{...BOX({padding:"11px 10px",textAlign:"center",marginBottom:0})}}>
                      ...
                      <div style={{fontSize:9,color:C.muted,marginTop:2,letterSpacing:2}}>{s.label}</div>
                    </div>
                  ))}
                </div>
                {[
                  ...
                  <div key={f.label} style={BOX()}>
                    <div style={{fontSize:10,color:C.muted,letterSpacing:3,marginBottom:7}}>{f.label}</div>
            ...
            {smokeTab==="breathe"&&(
              <div style={{textAlign:"center",paddingTop:12}}>
                <div style={{fontSize:10,color:C.muted,letterSpacing:4,marginBottom:18}}>BOX BREATHING — TACTICAL CALM</div>
            ...
            {smokeTab==="badges"&&(
              <div>
                <div style={{fontSize:10,color:C.muted,letterSpacing:3,marginBottom:10,marginTop:8}}>{badges.length}/{ACHIEVEMENTS.length} COMMENDATIONS</div>
                  ...
                  {ACHIEVEMENTS.map(a=>{
                    ...
                    return(
                      <div key={a.id} style={{...BOX({marginBottom:0,textAlign:"center",padding:13,opacity:done?1:0.28,background:done?"rgba(201,162,39,0.07)":C.sub,borderColor:done?"rgba(201,162,39,0.3)":C.border})}}>
                        ...
                        <div style={{fontSize:10,fontWeight:"bold",color:done?C.gold:C.muted,letterSpacing:1}}>{a.title.toUpperCase()}</div>
                        {!done&&<div style={{fontSize:10,color:C.muted,marginTop:3}}>🔒 LOCKED</div>}
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{display:"flex",gap:4,margin:"10px 0",overflowX:"auto",paddingBottom:2}}>
              {PILLARS.map(p=>(
                <button key={p.id} onClick={()=>setActivePillar(p.id)}
                  style={{padding:"5px 10px",borderRadius:3,background:activePillar===p.id?`${p.color}15`:"rgba(255,255,255,0.02)",border:activePillar===p.id?`1px solid ${p.color}50`:`1px solid ${C.border}`,color:activePillar===p.id?p.color:C.muted,fontSize:10,cursor:"pointer",letterSpacing:2,whiteSpace:"nowrap",fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace"}}>
                  {p.icon} {p.label}
                </button>
              ))}
            </div>
              ...
              return(
                <div key={h.id} onClick={()=>toggleHabit(h.id)}
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:10,color:C.dim,marginTop:2}}>{h.desc}</div>
                  </div>
                </div>
              );
            })}
            <div style={{...BOX({textAlign:"center",marginTop:2})}}>
              <div style={{fontSize:10,letterSpacing:3,color:C.muted,marginBottom:3}}>PILLAR STATUS</div>
              ...
              <div style={{fontSize:10,color:C.muted,letterSpacing:2}}>OBJECTIVES COMPLETE</div>
            </div>
          </div>
        )}
        ...
        {tab==="calendar"&&(
          <div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",padding:"7px 10px",background:C.sub,border:`1px solid ${C.border}`,borderRadius:6,marginBottom:8}}>
              {[{c:`rgba(143,196,58,0.45)`,l:"80-100%"},{c:`rgba(201,162,39,0.4)`,l:"40-79%"},{c:`rgba(192,57,43,0.35)`,l:"1-39%"},{c:"rgba(255,255,255,0.03)",l:"No log"}].map(x=>(
                <div key={x.l} style={{display:"flex",alignItems:"center",gap:4}}>
                  ...
                  <span style={{fontSize:9,color:C.muted,letterSpacing:1}}>{x.l}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:3}}>
              {DAY_NAMES.map((d,i)=>(
                <div key={i} style={{textAlign:"center",fontSize:10,color:C.muted,letterSpacing:1,padding:"3px 0"}}>{d}</div>
              ))}
            </div>
              ...
              {Array.from({length:daysInMonth(calYear,calMonth)}).map((_,i)=>{
                ...
                return(
                  <div key={key} onClick={()=>setSelectedDay(isSel?null:key)}
                    ...
                    <div style={{fontSize:11,fontWeight:isToday?"bold":"normal",color:isToday?C.accent:C.text}}>{day}</div>
                    {score>0&&<div style={{fontSize:8,color:isToday?C.accent:C.dim,marginTop:1}}>{score}%</div>}
                  </div>
                );
              })}
            </div>
              ...
              return(
                <div style={{...BOX({background:"rgba(201,162,39,0.05)",borderColor:"rgba(201,162,39,0.18)"})}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{fontSize:10,letterSpacing:3,color:C.gold}}>FIELD REPORT — {selectedDay}</div>
                    <div style={{fontSize:18,fontWeight:900,color:score>=80?C.accent:score>=40?C.gold:score>0?C.alert:C.muted}}>{score}%</div>
                  </div>

                  {completed.length>0&&(
                    <div style={{marginBottom:8}}>
                      <div style={{fontSize:9,letterSpacing:3,color:C.accent,marginBottom:6}}>✓ COMPLETED</div>
                      {completed.map(hab=>(
                        <div key={hab.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:`1px solid ${C.border}`}}>
                          ...
                          <span style={{fontSize:11,color:C.text}}>{hab.label}</span>
                          <span style={{fontSize:9,color:C.muted,marginLeft:"auto",letterSpacing:1}}>{hab.pillarLabel}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {missed.length>0&&selectedDay===today&&(
                    <div style={{marginBottom:8}}>
                      <div style={{fontSize:9,letterSpacing:3,color:C.alert,marginBottom:6}}>✗ PENDING</div>
                      {missed.map(hab=>(
                        <div key={hab.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:`1px solid ${C.border}`,opacity:0.5}}>
                          ...
                          <span style={{fontSize:11,color:C.muted}}>{hab.label}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {completed.length===0&&missed.length>0&&selectedDay!==today&&(
                    <div style={{fontSize:11,color:C.muted,textAlign:"center",padding:"10px 0",letterSpacing:2}}>NO OBJECTIVES LOGGED THIS DAY</div>
                  )}

                  {dayJ.length>0&&(
                    <div style={{marginTop:8}}>
                      <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginBottom:5}}>📓 JOURNAL</div>
                      {dayJ.map((e,i)=>(
                        <div key={i} style={{fontSize:11,color:C.dim,fontStyle:"italic",lineHeight:1.7,padding:"4px 0",borderBottom:`1px solid ${C.border}`}}>"{e.text}"</div>
                      ))}
                    </div>
                  )}
                  {dayN.length>0&&(
                    <div style={{marginTop:8}}>
                      <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginBottom:5}}>⚙️ NOTEBOOK</div>
                      {dayN.map((e,i)=>(
                        <div key={i} style={{fontSize:11,color:C.dim,lineHeight:1.7,padding:"4px 0",borderBottom:`1px solid ${C.border}`}}>{e.text}</div>
                      ))}
                    </div>
                  )}
                </div>
...
- [ ] 12:00 Lunch`}</pre>
        ...
        {tab==="intel"&&(
          <div>
            <div style={{fontSize:10,letterSpacing:4,fontWeight:700,color:C.gold,marginTop:12,marginBottom:8}}>NIGHTLY FIELD REPORT</div>
            <textarea value={journalText} onChange={e=>setJournalText(e.target.value)}
              placeholder="What went well today? What to improve? How did you live your identity?"
              style={{width:"100%",minHeight:108,padding:"9px 11px",borderRadius:6,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,color:C.text,fontSize:11,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace",resize:"vertical",boxSizing:"border-box",lineHeight:1.7}}/>
            ...
            <div style={{fontSize:10,letterSpacing:4,color:"#4a9eff",marginBottom:8}}>⚙️ ENGINEERING NOTEBOOK</div>
            <textarea value={notebookText} onChange={e=>setNotebookText(e.target.value)}
              ...
              style={{width:"100%",minHeight:88,padding:"9px 11px",borderRadius:6,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,color:C.text,fontSize:11,fontFamily:"ui-monospace,'Cascadia Code','Segoe UI Mono',Consolas,'Roboto Mono',monospace",resize:"vertical",boxSizing:"border-box",lineHeight:1.7}}/>
            ...
            {journalEntries.length>0&&(
              <div>
                <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginBottom:7}}>PAST REPORTS</div>
                {journalEntries.slice(0,5).map((e,i)=>(
                  <div key={i} style={{...BOX({marginBottom:7})}}>
                    <div style={{fontSize:11,color:C.dim,fontStyle:"italic",lineHeight:1.7}}>"{e.text}"</div>
                    <div style={{fontSize:9,color:C.muted,marginTop:4,letterSpacing:1}}>{e.date}</div>
                  </div>
                ))}
              </div>
            )}
            {notebookEntries.length>0&&(
              <div style={{marginTop:6}}>
                <div style={{fontSize:9,letterSpacing:3,color:C.muted,marginBottom:7}}>NOTEBOOK ENTRIES</div>
                {notebookEntries.slice(0,5).map((e,i)=>(
                  <div key={i} style={{...BOX({marginBottom:7,background:"rgba(74,158,255,0.04)",borderColor:"rgba(74,158,255,0.13)"})}}>
                    <div style={{fontSize:11,color:C.dim,lineHeight:1.7}}>{e.text}</div>
                    <div style={{fontSize:9,color:C.muted,marginTop:4,letterSpacing:1}}>{e.date}</div>
                  </div>
                ))}
              </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:`rgba(28,31,36,0.97)`,backdropFilter:"blur(14px)",borderTop:`1px solid ${C.border}`,display:"grid",gridTemplateColumns:"repeat(6,1fr)",padding:"7px 0 14px",zIndex:100}}>
        ...
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"3px 0"}}>
            ...
            <span style={{fontSize:9,letterSpacing:2,color:tab===t.id?C.accent:C.muted,fontWeight:tab===t.id?700:400}}>{t.label}</span>
